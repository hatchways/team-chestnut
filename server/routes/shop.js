...
router.post("/item/delete/:itemid", verify, async (req, res, next) => {
  const itemid = req.params.itemid;
  const token = req.header("auth-token");
  const imageUrl = req.body.image;
  const key = imageUrl.split('/').slice(-1)[0];
  const response = deleteImage(key); 
  const item = await items.findById(itemid, { __v: false });

response.then((data,rej) =>{
  ...
  if(data.Deleted[0].Key === key) {
    ...
    item.save().then((response,error)=>{
      if(response){
        return res.header("auth-token", token).status(200).send({ item: response });
      } else if (error){
    ...
    return res.status(400).send({ message: "There was an error trying to delete the image" });
  } else {
